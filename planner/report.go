...
// RenderZUITree prints a visual tree of elements and their view placements.
func RenderZUITree(w io.Writer, ws *workspace.Workspace) {
	fmt.Fprintln(w)
	fmt.Fprintln(w, "## ZUI Navigation Tree")
	fmt.Fprintln(w)
	...
	rootChildren := children[syntheticRootViewRef]
	if len(rootChildren) == 0 {
		fmt.Fprintln(w, "⚠️  No elements are placed in the \"root\" view. The diagram will be empty upon initial load.")
	} else {
		fmt.Fprintln(w, "Root")
		visited := make(map[string]bool)
		for i, ref := range rootChildren {
			isLast := i == len(rootChildren)-1
			printZUITreeNode(w, ws, children, ref, "", isLast, visited)
		}
		...
		if len(disconnected) > 0 {
			...
			fmt.Fprintln(w)
			fmt.Fprintln(w, "⚠️  Disconnected Elements:")
...
func printZUITreeNode(w io.Writer, ws *workspace.Workspace, children map[string][]string, ref string, prefix string, isLast bool, visited map[string]bool) {
	if visited[ref] {
		// Avoid cycles though validation should catch them
		return
	}
	visited[ref] = true
	element := ws.Elements[ref]
	...
	fmt.Fprintf(w, "%s%s%s: %s%s\n", prefix, marker, ref, element.Name, viewSuffix)
	...
	nodeChildren := children[ref]
	for i, childRef := range nodeChildren {
		last := i == len(nodeChildren)-1
		printZUITreeNode(w, ws, children, childRef, newPrefix, last, visited)
	}
}
