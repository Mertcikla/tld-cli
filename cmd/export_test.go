...
func TestExportCmd(t *testing.T) {
	...
	if !strings.Contains(stdout, "Exported 1 diagrams, 1 objects") {
	...
	if _, err := os.Stat(filepath.Join(dir, "diagrams.yaml")); os.IsNotExist(err) {
		t.Error("diagrams.yaml not created")
	}
	if _, err := os.Stat(filepath.Join(dir, "objects.yaml")); os.IsNotExist(err) {
		t.Error("objects.yaml not created")
	}
}
