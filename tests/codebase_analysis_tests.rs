...
fn test_analyze_all_codebases() {
    ...
    for lang in languages {
        ...
        cmd.arg("-w").arg(wdir).arg("analyze").arg(format!("tests/test-codebase/{}", lang));
        
        // Ensure success
        let output = cmd.output().expect("Failed to execute command");
        if !output.status.success() {
             eprintln!("Command failed for {}: {}", lang, String::from_utf8_lossy(&output.stderr));
             panic!("Command failed");
        }
        
        // 1. Verify elements.yaml exists and is not empty
        let elements_path = dir.path().join("elements.yaml");
        assert!(elements_path.exists(), "elements.yaml should exist for {} codebase", lang);
        let elements_data = fs::read_to_string(&elements_path).expect("Failed to read elements.yaml");
        assert!(!elements_data.trim().is_empty(), "elements.yaml should not be empty for {} codebase", lang);
