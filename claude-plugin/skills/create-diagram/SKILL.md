...
> **Prerequisite:** `tld` must be on your PATH. If it's not found, prompt the user to install it:
> ```bash
...
This is not architecture diagramming in the traditional sense. It is a **navigatable atlas of the entire codebase** — like a hyperlinked wiki where you can zoom into anything. At the top you see services talking to each other. Click into a service and you see its modules. Click into a module and you see its classes. Click into a class and you see its xparameters, its methods, what each method calls, its parent classes, its subclasses, the interfaces it implements.
...
Before exploring, ask the user for their input on two questions:
...
### Dispatch a code-explorer agent

Do not explore the codebase yourself. Dispatch a **code-explorer agent** and give it a clear brief:

```
Explore the codebase at [path]. Produce a structured report with:

1. Directory tree (top 2 levels)
2. Entry points (main files, server bootstraps, CLI roots)
3. Major layers / subsystems — for each:
   - Name and one-line purpose
   - Key files / packages
   - What it calls (outbound dependencies)
   - What calls it (inbound dependencies)
   - Shared infrastructure it uses (DBs, queues, caches, auth, etc.)
4. External systems (third-party APIs, cloud services, message brokers)
5. Config and environment surface (env vars, config files, feature flags)

The output of this report is not a list of files — it is a map of connections.
For every subsystem, answer: what does it call? what calls it? what data does it read or write?
```

Run one agent per top-level area if the codebase has distinct roots (e.g. `backend/`, `frontend/`, `infra/`). Run them in parallel.

## Step 2: Produce a subsystem inventory

Read every agent report. Your job as orchestrator is to synthesise the bigger picture — how the parts fit together across the whole system, not just within each area.

For each subsystem in the reports, write an inventory entry:
...
### Assess the reports before accepting them

After reading each report, ask yourself:

- **Is this subsystem suspiciously thin?** One sentence for a package with dozens of files, no edges listed, dependencies described as "various" — this is under-explored. Dispatch a focused follow-up agent for that component with a tighter scope.
- **Is this subsystem too broad to map at once?** A single report entry covering a large domain (e.g. "all auth logic" spanning 30+ files) cannot produce good edges. Split it: dispatch one agent per major sub-area and re-integrate.
- **Are cross-subsystem connections missing?** Two subsystems that must interact but show no edges between them means at least one agent missed something. Dispatch a connector agent scoped to the interface between them.

A follow-up agent prompt looks like:

```
The previous exploration of [subsystem] was too coarse. Explore [specific path or scope] in detail.
Focus on: [what was thin or missing]. Return the same report format as before.
```

Dispatch follow-up agents until every subsystem has:
- A clear purpose statement
- At least one inbound dependency (or is an entry point)
- At least one outbound dependency (or is a leaf)
- Named interactions with shared infrastructure

Do not proceed to Step 3 until every top-level directory or package is accounted for in the inventory. A diagram that covers 60% of the codebase is worse than no diagram — it creates false confidence.
