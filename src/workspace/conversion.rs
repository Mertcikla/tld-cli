...
pub fn from_export_response(
    ...
    for e in msg.elements {
        let ref_name = existing_element_refs.get(&e.id).cloned()
        ...
        new_ws.elements.insert(ref_name.clone(), Element {
            name: e.name,
            kind,
        ...
        if let Some(meta) = &mut new_ws.meta {
            if let Some(ts) = e.updated_at {
                meta.elements.insert(ref_name, ResourceMetadata {
                    id: e.id,
        ...
        if owner_ref != "root" {
            ...
            if let Some(meta) = &mut new_ws.meta {
                if let Some(ts) = d.updated_at {
                    meta.views.insert(owner_ref, ResourceMetadata {
                        id: d.id,
                        updated_at: Utc.timestamp_opt(ts.seconds, ts.nanos as u32).unwrap(),
                        conflict: false,
                    });
    ...
    for p in msg.placements {
        if let Some(element_ref) = object_id_to_ref.get(&p.element_id) {
            let parent_ref = diagram_id_to_view_ref.get(&p.view_id).cloned().unwrap_or_else(|| "root".to_string());
    ...
    for c in msg.connectors {
        let view_ref = diagram_id_to_view_ref.get(&c.view_id).cloned().unwrap_or_else(|| "root".to_string());
        ...
        if let (Some(s), Some(t)) = (src_ref, tgt_ref) {
            ...
            new_ws.connectors.insert(key.clone(), Connector {
                view: view_ref,
                source: s.clone(),
                target: t.clone(),
                label,
            ...
            if let Some(meta) = &mut new_ws.meta {
                if let Some(ts) = c.updated_at {
                    meta.connectors.insert(key, ResourceMetadata {
                        id: c.id,
                        updated_at: Utc.timestamp_opt(ts.seconds, ts.nanos as u32).unwrap(),
                        conflict: false,
                    });
