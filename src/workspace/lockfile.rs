...
pub fn load_metadata(dir: &str) -> Result<Meta, TldError> {
    ...
    if meta.elements.is_empty() {
        if let Ok(m) = load_yaml_metadata_section(dir, "elements.yaml", "_meta_elements") {
            meta.elements = m;
        }
    }
    if meta.views.is_empty() {
        if let Ok(m) = load_yaml_metadata_section(dir, "elements.yaml", "_meta_views") {
            meta.views = m;
        }
    }
    if meta.connectors.is_empty() {
        if let Ok(m) = load_yaml_metadata_section(dir, "connectors.yaml", "_meta_connectors") {
            meta.connectors = m;
        }
    }
